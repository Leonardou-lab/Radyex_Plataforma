...
Spec de dónde va a salir cada campo cuando se conecte `radyex-web/lib/data.ts`
a datos reales (Fase 4). Se generó al renombrar los campos de `lib/data.ts` de
inglés a español (2026-08-28) para que ese renombrado se hiciera **una sola
vez**: los nombres que usa hoy la UI ya coinciden con las columnas reales de
...
| `pacienteDesde` | — | CALCULADO/DISPLAY (texto compuesto, sin columna propia — a definir cómo se deriva al conectar datos reales, p. ej. de la orden más antigua del paciente) |
| `archivos` | — | CALCULADO/DISPLAY (agrupación por año de filas de `archivos` de esa orden) |
